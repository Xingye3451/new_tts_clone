...
class SparkTTS:
    ...
    def _initialize_inference(self):
        ...
        # # 如果有需要添加的token，则扩展tokenizer
        # if tokens_to_add:
        #     logger.info(f"添加以下特殊token到tokenizer: {tokens_to_add}")
        #     self.tokenizer.add_special_tokens(
        #         {"additional_special_tokens": tokens_to_add}
        #     )
        #     # 注意：如果添加了新token，您还需要调整模型的embedding层大小
        #     # self.model.resize_token_embeddings(len(self.tokenizer))

        self.model = AutoModelForCausalLM.from_pretrained(f"{self.model_dir}/LLM")

        # # 如果添加了新token，调整模型的embedding层大小
        # if tokens_to_add:
        #     self.model.resize_token_embeddings(len(self.tokenizer))
    ...
    def inference(
        ...
        if gender is not None:
            ...
            # 打印日志
            print(f"使用prompt模式, prompt_text: {prompt_text}")
